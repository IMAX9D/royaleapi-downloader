...
本仓库把可复用下载模块、长期运行能力、离线测试与原部署工具放在同一个 Python 包中。文件多是因为这些职责分开实现；正常下载只需要运行一个采集入口。
...
源码目录保持现有模块路径，方便已有命令和导入继续使用。导航中的“旧部署”表示需要额外环境，不表示可以随意删除：例如核心代码仍会导入部分高级校验或共享辅助函数。
